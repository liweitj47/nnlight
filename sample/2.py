...
class SimpleLayer(Layer):
    def __init__(self, name, inputs, params):
        ...
        if len(self.W.shape()) != 2:
            self.error_("weight matrix for logistic layer '%s' should be of dimension 2" % self.name)
        ...
        self.y = NNArrayFloat32(
            ...
            shape=[self.x.shape()[0], self.W.shape()[1]]
        )
    ...
    def node(self, ref=None):
        if ref is None:
            ...
            self.error_("simple layer '%s' has no attribute '%s'" % (self.name, ref))
...
class SequencialSoftmaxLayer(Layer):
    def __init__(self, name, inputs, params):
        Layer.__init__(self)
        ...
        self.y, _ = theano.scan(
            fn = lambda x: theano.tensor.nnet.softmax(
                             theano.tensor.dot(x, self.W) + self.b
                           ),
            sequences = [self.x]
        )
    ...
    def shape(self):
        return [None, None, self.W_shape[1]]
...
class SentenceConvLayer(Layer):
    def __init__(self, name, inputs, params):
        ...
        kernel_width = self.kernel.shape()[2]
        ...
        reduced_conv = theano.tensor.nnet.sigmoid(
            theano.tensor.sum(raw_conv, axis=2)
...
class MaxPoolingLayer(Layer):
    def __init__(self, name, inputs, params):
        ...
        self.contrib = NNArrayInt64(v=theano.tensor.argmax(self.data.value(), axis=1),
...
class LstmLayer(Layer):
    def __init__(self, name, inputs, params):
        ...
        x, mask, W_i, b_i, W_f, b_f, W_o, b_o, W_c, b_c = [v.value() for v in inputs]

        initial_value = theano.tensor.zeros_like(
            theano.tensor.dot(
                theano.tensor.sum(x, axis=[1,2]).dimshuffle(0, 'x'),
                W_c
            )
        )


        # x: [sentence_length * batch_size * wordvec_length]
        data_wordvec = x.dimshuffle(2, 0, 1)
        data_mask = mask.dimshuffle(1, 0)

        def _step(wordvec, wordmask, h, c):

            state = theano.tensor.concatenate([wordvec, h], axis=1)
            wordmask = wordmask.dimshuffle(0, 'x')

            forget_gate = theano.tensor.nnet.sigmoid(
                theano.tensor.dot(state, W_f) + b_f
            )

            input_gate = theano.tensor.nnet.sigmoid(
                theano.tensor.dot(state, W_i) + b_i
            )

            c2 = theano.tensor.tanh(
                theano.tensor.dot(state, W_c) + b_c
            )

            c2 = forget_gate * c + input_gate * c2

            output_gate = theano.tensor.nnet.sigmoid(
                theano.tensor.dot(state, W_o) + b_o
            )

            h2 = output_gate * theano.tensor.tanh(c2)

            final_h = wordmask*h2 + (1-wordmask)*h
            final_c = wordmask*c2 + (1-wordmask)*c
        ...
        outputs, updates = theano.scan(
            _step,
            sequences = [data_wordvec, data_mask],
            outputs_info = [initial_value, initial_value],
        )

        final = outputs[0].dimshuffle(1, 0, 2) #*  mask.dimshuffle(0, 1, 'x')

        self.final = NNArrayFloat32(v=final[:,-1],
                                    shape=[inputs[0].shape()[0], inputs[1].shape()[1]])
        self.out = NNArrayFloat32(v=final,
                                  shape=[inputs[0].shape()[0], inputs[0].shape()[2], inputs[1].shape()[1]])
    ...
    def node(self, ref=None):
        if ref is None:
            ...
            self.error_("lstm layer '%s' has no attribute '%s'" % (self.name, ref))
...
class LowRankTensorLayer(Layer):
    def __init__(self, name, inputs, params):
        Layer.__init__(self)
        ...
        if len(inputs) != 4:
            self.error_("low_rank_tensor layer '%s' requires exactly 4 input" % self.name)
        if not all([isinstance(x, NNValue) for x in inputs]):
            self.error_("inputs for layer '%s' should be of 'NNValue' type" % self.name)

        self.data_l = inputs[0]
        if len(self.data_l.shape()) != 2:
            self.error_("data for low_rank_tensor layer '%s' should be of dimension 2" % self.name)
        self.data_r = inputs[1]
        if len(self.data_r.shape()) != 2:
            self.error_("data for low_rank_tensor layer '%s' should be of dimension 2" % self.name)
        self.W_l = inputs[2]
        if len(self.W_l.shape()) != 3:
            self.error_("left weight for low_rank_tensor layer '%s' should be of dimension 3" % self.name)
        self.W_r = inputs[3]
        if len(self.W_r.shape()) != 3:
            self.error_("right weight for low_rank_tensor layer '%s' should be of dimension 3" % self.name)

        h_l = theano.tensor.tensordot(self.data_l.value(), self.W_l.value(), [[1],[1]])
        h_r = theano.tensor.tensordot(self.data_r.value(), self.W_r.value(), [[1],[1]])

        self.y = NNArrayFloat32(
            v = theano.tensor.nnet.sigmoid(
                        theano.tensor.sum(h_l * h_r, axis=2)
                    ),
            shape = [self.data_l.shape()[0], self.W_l.shape()[0]]
        )
    ...
    def node(self, ref=None):
        if ref is None:
            ...
            self.error_("low_rank_tensor layer '%s' has no attribute '%s'" % (self.name, ref))
...
class MaxMarginLoss(Loss):
    def __init__(self, name, inputs, params):
        ...
        if len(inputs) != 2:
            self.error_("MaxMarginLoss need inputs: [pos, neg]")
        if not all([isinstance(x, NNValue) for x in inputs]):
            self.error_("inputs for loss '%s' should be of 'NNValue' type" % self.name)

        self.pos, self.neg = inputs[0], inputs[1]
        if len(self.pos.shape()) != 1 or len(self.neg.shape()) != 1:
            self.error_("input data for MaxMarginLoss should be of dimension 1")

        margin = params.get("margin", 0.2)
        if not isinstance(margin, NNScalarFloat32):
            self.error_("margin for MaxMarginLoss should be of float type")

        diff = self.pos.value() - self.neg.value()
        self.loss = NNScalarFloat32(
            -theano.tensor.mean(
                theano.tensor.switch(
                    theano.tensor.lt(diff, margin.value()), diff, 0.0
                )
            )
        )
        self.count = NNScalarInt64(
            theano.tensor.sum(
                theano.tensor.switch(
                    theano.tensor.lt(diff, margin.value()), 1, 0
                )
            )
        )
...
class CrossEntropyLoss(Loss):
    def __init__(self, name, inputs, params):
        ...
        x, y = self.x.value(), self.y.value()
        logx = theano.tensor.switch(theano.tensor.eq(x, 0.), math.log(0.00000001), theano.tensor.log(x))
        prob = theano.tensor.sum(y * logx, axis=dims-1)
        predict = theano.tensor.argmax(
            x, axis=dims-1
        )

        if len(inputs) == 2:
            loss = -theano.tensor.mean(prob)
        else:
            mask = inputs[2].value()
            divid = theano.tensor.sum(mask)
            loss = -theano.tensor.sum(prob * mask) / divid

        self.prob = NNArrayFloat32(shape=self.x.shape()[:dims-1], v=prob)
        self.loss = NNScalarFloat32(loss)
        self.predict = NNArrayInt64(shape=self.x.shape()[:dims-1], v=predict)
...
class NNBase:
    ...
    @staticmethod
    def __error(text):
        NNDebug.error_("[Network] " + text)

    @staticmethod
    def __assert(cond, msg):
        NNDebug.assert_(cond, "[Network] " + msg)

    @staticmethod
    def __warning(text):
        NNDebug.warning_("[Network] " + text)
    ...
    def __check_data(self, name, data):
        ...
        self.__assert(isinstance(data, numpy.ndarray), "data '%s' should be numpy.ndarray" % name)
    ...
    def train(self, data_dicts, iters=None, batch=None,
        ...
        for i in range(iters):
            ...
            for data_dict in data_dicts:
                for name in data_dict:
                    self.set_data(name, data_dict[name])

                start = 0
                end = data_dict.values()[0].shape[0]

                if batch == None:
                    batch = end
                batch_num = end / batch + 1
    ...
    def get_data(self, name):
        ...
        return self.get_core()["data"][name].raw()
    ...
    def set_data(self, name, data):
        ...
        self.get_core()["data"][name].update(data)
